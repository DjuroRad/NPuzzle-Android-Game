package com.example.npuzzle.hwk7;
...
*/
	...
	*/
    ...
    public boolean moveIfPossible( int row, int column ){
        if( !( row != empty_row && column != empty_col ) ){
            if( row != empty_row && column == empty_col ){
            ...
            else if( column != empty_col && row == empty_row ){
                if( column == empty_col + 1 || column == empty_col -1 ){
                    if( column == empty_col - 1 ){
                        move( 'L' );
                    }
                    else{
                        move( 'R' );
